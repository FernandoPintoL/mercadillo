...
const model_service = SectorService;
...
onMounted(() => {
    if (props.model != null) {
        ...
        form.maximo = props.model.maximo;
        form.minimo = props.model.minimo;
    }
});
...
const validateMaximo = (e) => {
    ...
    if (!validacion.validarNumero(e.target.value)) {
        datas.maximoError = 'Solo se permiten números.';
    } else {
        form.maximo = e.target.value;
        datas.maximoError = '';
    }
};
...
const validateMinimo = (e) => {
    ...
    if (!validacion.validarNumero(e.target.value)) {
        datas.minimoError = 'Solo se permiten números.';
    } else {
        form.minimo = e.target.value;
        datas.minimoError = '';
    }
};
...
const handleErrors = (error) => {
    if (error.response.data && error.response.data.statusCode === 422) {
        ...
        datas.direccionError = errors.direccion ? errors.direccion[0] : '';
        datas.maximoError = errors.maximo ? errors.maximo[0] : '';
        datas.minimoError = errors.minimo ? errors.minimo[0] : '';
    } else {
    ...
    <AppLayout :title="props.isCreate ? 'Crear '+model_path : 'Editar '+model_path">
        <div class="mx-auto px-4 py-6">
            ...
            <div>
                ...
                <button v-if="props.editar || props.crear"
                        type="submit"
                        @click="submit_create"
